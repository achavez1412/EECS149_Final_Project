...
while(True):
    ...
    if msg:
        ...
        max_temp, min_temp = max(max_temp, temp), min(min_temp, temp)
        hum = hex_str_to_float(msg.decode()[8:16])
        hum_arr.append(hum)
        if(len(hum_arr) > 4):
            hum_arr.pop(0)
        count = int_str_to_float(msg.decode()[16:24])
        print("temp: " + str(temp) + " max_temp: " + str(max_temp) + " min_temp: " + str(min_temp))
        print("hum: " + str(hum) + " running average hum: " + str(sum(hum_arr)/len(hum_arr)))
